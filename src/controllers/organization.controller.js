...
async function createOrganization(req, res) {
    ...
    return res.status(201).json({message: "Organization created successfully", body: req.body});
    } catch (error) {
...
async function updateOrganization(req, res) {
    try {
        ...
        const body = (rawBody && typeof rawBody === 'object' && rawBody.organization && typeof rawBody.organization === 'object')
        ...
        if (!result || result.affectedRows === 0) {
            return res.status(404).json({ message: "Organization not found (no rows updated)", organization_id: orgId });
        }
