...
module.exports = () => {
  return async (req, res, next) => {
    try {
      ...
      // Bypass for Global SUPER_ADMIN
      const [userRes] = await db.query(
        "SELECT code FROM user WHERE user_id = ?",
        [userId],
      );
      if (
        userRes.length > 0 &&
        (userRes[0].code === "SUPER_ADMIN" || userRes[0].code === "ADMIN")
      ) {
        return next();
      }
      ...
      return res
        .status(403)
        .json({ error: "Access denied: Insufficient organization scope" });
    } catch (error) {
